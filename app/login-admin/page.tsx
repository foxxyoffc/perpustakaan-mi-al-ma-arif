    ...
    try {
      ...
      if (!response.ok || !data.success) {
        ...
        if (data.locked || data.reportCreated) {
          setError(
            data.message ||
              "Terlalu banyak percobaan login gagal. Aktivitas telah dilaporkan kepada developer."
          );
        } else {
      ...
      setSuccess("Login admin berhasil. Mengarahkan ke dashboard...");

      const redirectTo = data.redirect || "/admin";
        ...
        <div className="rounded-3xl border border-emerald-900/10 bg-white p-6 shadow-[0_20px_70px_rgba(15,61,36,0.10)] sm:p-8">
          ...
          <div className="mb-7">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-500">
              Halaman khusus administrator perpustakaan sekolah.
            </p>
          </div>

          {/* Security notice */}
          <div className="mb-5 flex gap-3 rounded-2xl border border-amber-100 bg-amber-50/70 p-4">
            <div className="mt-0.5 shrink-0 text-amber-700">
              <InfoIcon className="h-5 w-5" />
            </div>

            <p className="text-xs leading-5 text-amber-800">
              Gunakan akun administrator yang diberikan oleh
              developer. Jangan membagikan username atau password
              kepada pihak lain.
            </p>
          </div>
...
function AdminIcon({
  ...
  return (
    ...
    >
      ...
      <path d="M8.5 16c.6-1.5 1.8-2.3 3.5-2.3s2.9.8 3.5 2.3" />
    </svg>
  );
}
